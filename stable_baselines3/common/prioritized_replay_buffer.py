...
class PrioritizedReplayBuffer(DictReplayBuffer):
    ...
    def sample(self, batch_size: int, env: VecNormalize | None = None) -> DictReplayBufferSamples:
        ...
        obs: dict = self._normalize_obs({key: obs[sample_indices, env_indices, :] for key, obs in self.observations.items()}, env)
        next_obs: dict = self._normalize_obs({key: obs[sample_indices, env_indices, :] for key, obs in self.next_observations.items()}, env)

        obs = {key: self.to_torch(o) for key, o in obs.items()}
        next_obs = {key: self.to_torch(o) for key, o in next_obs.items()}

        batch = (
            obs,
            self.to_torch(self.actions[sample_indices, env_indices, :]),
            next_obs,
            self.to_torch(self.dones[sample_indices]),
            self.to_torch(self.rewards[sample_indices]),
            self.to_torch(weights),
        )
        return DictReplayBufferSamples(*batch, leaf_nodes_indices)  # type: ignore[arg-type,call-arg]
